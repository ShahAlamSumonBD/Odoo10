...
class NewIndentIndent(models.Model):
    _name = 'new.indent.indent'
    _description = 'Indent'
    _order = "approve_date desc"

    name = fields.Char('Indent #',size=256, track_visibility='always')
    ...
    type = fields.Selection([('gen', 'General Item')], 'Type',
                            required=True,
                            track_visibility='onchange')
    product_lines = fields.One2many('new.indent.product.lines', 'indent_id',
                                    string='Products',
    ...
    item_for = fields.Selection([('store', 'Store'), ('capital', 'Capital')], 'Purchase for',
                                readonly=True, states={'draft': [('readonly', False)]})
    amount_total = fields.Float(digits=(20, 4), compute='_computed_total_amount',
                                string='Total', store=True)
    amount_flag = fields.Boolean(default=False)
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirm'),
                              ('waiting_approval', 'Waiting for Approval'),
                              ('inprogress', 'In Progress'),
                              ('received', 'Received'), ('reject', 'Rejected'),
                              ('cancel', 'Cancel'), ('close', 'Close')],
                             'State', default='draft')
    approver_id = fields.Many2one('res.users', 'Authority', readonly=True,
    ...
    move_type = fields.Selection([('direct', 'Partial'), ('one', 'All at once')], 'Receive Method',
                                 track_visibility='onchange', readonly=True, required=True,
    ...
    def unlink(self):
        for line in self:
            if line.state != "draft":
                ...
                return super(NewIndentIndent, self).unlink()
    ...
    def _prepare_indent_picking(self, indent):
        ...
        res = {
            ...
            'location_dest_id': location_id or 1,
    ...
    def _prepare_indent_line_move(self, indent, line, picking_id, date_planned):
        ...
        res = {
            ...
            'location_id': location_id or 1,
            'location_dest_id': indent.department_id.id or 1,
            'state': 'confirmed',
...
class NewStockPicking(models.Model):
    _inherit = 'stock.picking'

    @api.one
    def action_stock_picking_confirm(self, id):
        # Implement method that will check further verification for authority
        return super(NewStockPicking, self).action_stock_picking_confirm(id)

    @api.multi
    def do_transfer(self):
        res = super(NewStockPicking, self).do_transfer()
        if res:
            picking = self.browse(self.ids)[0]
            indent_obj = self.env['new.indent.indent']
